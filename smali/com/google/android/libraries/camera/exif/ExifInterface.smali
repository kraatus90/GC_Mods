.class public Lcom/google/android/libraries/camera/exif/ExifInterface;
.super Ljava/lang/Object;
.source "PG"

# interfaces
.implements Lkke;


# annotations
.annotation build Lcom/google/android/apps/common/proguard/UsedByReflection;
.end annotation


# static fields
.field public static final A:I

.field public static final B:I

.field public static final C:I

.field public static final D:I

.field public static final E:I

.field public static final F:I

.field public static final G:I

.field public static final H:I

.field public static final I:I

.field public static final J:I

.field public static final K:I

.field public static final L:I

.field public static final M:I

.field public static final N:I

.field public static final O:I

.field public static final P:I

.field public static final TAG_SOFTWARE:I
    .annotation build Lcom/google/android/apps/common/proguard/UsedByReflection;
    .end annotation
.end field

.field private static final U:Ljava/nio/ByteOrder;

.field private static final V:Ljava/lang/Long;

.field private static final W:I

.field private static final X:I

.field private static final Y:I

.field private static final Z:I

.field public static final a:Ljava/lang/Long;

.field private static final aA:I

.field private static final aB:I

.field private static final aC:I

.field private static final aD:I

.field private static final aE:I

.field private static final aF:I

.field private static final aG:I

.field private static final aH:I

.field private static final aI:I

.field private static final aJ:I

.field private static final aK:I

.field private static final aL:I

.field private static final aM:I

.field private static final aN:I

.field private static final aO:I

.field private static final aP:I

.field private static final aQ:I

.field private static final aR:I

.field private static final aS:I

.field private static final aT:I

.field private static final aU:I

.field private static final aV:I

.field private static final aW:I

.field private static final aX:I

.field private static final aY:I

.field private static final aZ:I

.field private static final aa:I

.field private static final ab:I

.field private static final ac:I

.field private static final ad:I

.field private static final ae:I

.field private static final af:I

.field private static final ag:I

.field private static final ah:I

.field private static final ai:I

.field private static final aj:I

.field private static final ak:I

.field private static final al:I

.field private static final am:I

.field private static final an:I

.field private static final ao:I

.field private static final ap:I

.field private static final aq:I

.field private static final ar:I

.field private static final as:I

.field private static final at:I

.field private static final au:I

.field private static final av:I

.field private static final aw:I

.field private static final ax:I

.field private static final ay:I

.field private static final az:I

.field public static final b:I

.field private static final ba:I

.field private static final bb:I

.field private static final bc:I

.field private static final bd:I

.field private static final be:I

.field private static final bf:I

.field private static final bg:I

.field private static final bh:I

.field private static final bi:I

.field private static final bj:I

.field private static final bk:I

.field private static final bl:I

.field private static final bm:I

.field private static final bn:I

.field private static final bo:I

.field private static final bp:I

.field private static final bq:I

.field private static final br:I

.field private static final bs:I

.field private static final bt:I

.field private static final bu:I

.field private static final bv:I

.field private static bx:Ljava/util/HashSet;

.field private static by:Ljava/util/HashSet;

.field public static final c:I

.field public static final d:I

.field public static final e:I

.field public static final f:I

.field public static final g:I

.field public static final h:I

.field public static final i:I

.field public static final j:I

.field public static final k:I

.field public static final l:I

.field public static final m:I

.field public static final n:I

.field public static final o:I

.field public static final p:I

.field public static final q:I

.field public static final r:I

.field public static final s:I

.field public static final t:I

.field public static final u:I

.field public static final v:I

.field public static final w:I

.field public static final x:I

.field public static final y:I

.field public static final z:I


# instance fields
.field public Q:I

.field public R:Lkkd;

.field public final S:Ljava/text/DateFormat;

.field public final T:Ljava/util/Calendar;

.field private final bw:Ljava/text/DateFormat;

.field private bz:Landroid/util/SparseIntArray;


# direct methods
.method static constructor <clinit>()V
    .locals 2

    const/16 v0, 0x100

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->z:I

    const/16 v0, 0x101

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->y:I

    const/16 v0, 0x102

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->X:I

    const/16 v0, 0x103

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ad:I

    const/16 v0, 0x106

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aU:I

    const/16 v0, 0x10e

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->x:I

    const/16 v0, 0x10f

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->E:I

    const/16 v0, 0x110

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->G:I

    const/16 v0, 0x111

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->M:I

    const/16 v0, 0x112

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->H:I

    const/16 v0, 0x115

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bb:I

    const/16 v0, 0x116

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ba:I

    const/16 v0, 0x117

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->L:I

    const/16 v0, 0x11a

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->br:I

    const/16 v0, 0x11b

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bv:I

    const/16 v0, 0x11c

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aV:I

    const/16 v0, 0x128

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aZ:I

    const/16 v0, 0x12d

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bo:I

    const/16 v0, 0x131

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->TAG_SOFTWARE:I

    const/16 v0, 0x132

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->c:I

    const/16 v0, 0x13b

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->W:I

    const/16 v0, 0x13e

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bq:I

    const/16 v0, 0x13f

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aW:I

    const/16 v0, 0x211

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bs:I

    const/16 v0, 0x212

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bu:I

    const/16 v0, 0x213

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bt:I

    const/16 v0, 0x214

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aX:I

    const v0, 0x8298

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->af:I

    const v0, 0x8769

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->f:I

    const v0, 0x8825

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->p:I

    const v0, 0x10201

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->C:I

    const v0, 0x10202

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->D:I

    const v0, 0x2829a

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->h:I

    const v0, 0x2829d

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->l:I

    const v0, 0x28822

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->am:I

    const v0, 0x28824

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bi:I

    const v0, 0x28827

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->B:I

    const v0, 0x28828

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aT:I

    const v0, 0x29000

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aj:I

    const v0, 0x29003

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->e:I

    const v0, 0x29004

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->d:I

    const v0, 0x29101

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ab:I

    const v0, 0x29102

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ac:I

    const v0, 0x29201

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->K:I

    const v0, 0x29202

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->b:I

    const v0, 0x29203

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->Y:I

    const v0, 0x29204

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->g:I

    const v0, 0x29205

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aR:I

    const v0, 0x29206

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->N:I

    const v0, 0x29207

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aS:I

    const v0, 0x29208

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aQ:I

    const v0, 0x29209

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->i:I

    const v0, 0x2920a

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->j:I

    const v0, 0x29214

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bj:I

    const v0, 0x2927c

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->F:I

    const v0, 0x29286

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bp:I

    const v0, 0x29290

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bl:I

    const v0, 0x29291

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bn:I

    const v0, 0x29292

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bm:I

    const v0, 0x2a000

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ao:I

    const v0, 0x2a001

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aa:I

    const v0, 0x2a002

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->I:I

    const v0, 0x2a003

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->J:I

    const v0, 0x2a004

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aY:I

    const v0, 0x2a005

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->A:I

    const v0, 0x2a20b

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ap:I

    const v0, 0x2a20c

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bh:I

    const v0, 0x2a20e

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ar:I

    const v0, 0x2a20f

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->as:I

    const v0, 0x2a210

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aq:I

    const v0, 0x2a214

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bk:I

    const v0, 0x2a215

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ak:I

    const v0, 0x2a217

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bf:I

    const v0, 0x2a300

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->an:I

    const v0, 0x2a301

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->be:I

    const v0, 0x2a302

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->Z:I

    const v0, 0x2a401

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ag:I

    const v0, 0x2a402

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->al:I

    const v0, 0x2a403

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->P:I

    const v0, 0x2a404

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ai:I

    const v0, 0x2a405

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->k:I

    const v0, 0x2a406

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bd:I

    const v0, 0x2a407

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->at:I

    const v0, 0x2a408

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ae:I

    const v0, 0x2a409

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bc:I

    const v0, 0x2a40a

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bg:I

    const v0, 0x2a40b

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ah:I

    const v0, 0x2a40c

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->O:I

    const v0, 0x2a420

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aN:I

    const/high16 v0, 0x40000

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aM:I

    const v0, 0x40001

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->t:I

    const v0, 0x40002

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->s:I

    const v0, 0x40003

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->v:I

    const v0, 0x40004

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->u:I

    const v0, 0x40005

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->n:I

    const v0, 0x40006

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->m:I

    const v0, 0x40007

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->w:I

    const v0, 0x40008

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aG:I

    const v0, 0x40009

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aJ:I

    const v0, 0x4000a

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aE:I

    const v0, 0x4000b

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aC:I

    const v0, 0x4000c

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aI:I

    const v0, 0x4000d

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aH:I

    const v0, 0x4000e

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aL:I

    const v0, 0x4000f

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aK:I

    const v0, 0x40010

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->r:I

    const v0, 0x40011

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->q:I

    const v0, 0x40012

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aD:I

    const v0, 0x40013

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aA:I

    const v0, 0x40014

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->az:I

    const v0, 0x40017

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aw:I

    const v0, 0x40018

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->av:I

    const v0, 0x40019

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ay:I

    const v0, 0x4001a

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->ax:I

    const v0, 0x4001b

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aF:I

    const v0, 0x4001c

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->au:I

    const v0, 0x4001d

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->o:I

    const v0, 0x4001e

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aB:I

    const v0, 0x30001

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aO:I

    const v0, 0x30002

    sput v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->aP:I

    new-instance v0, Ljava/util/HashSet;

    invoke-direct {v0}, Ljava/util/HashSet;-><init>()V

    sput-object v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->by:Ljava/util/HashSet;

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->p:I

    int-to-short v1, v1

    invoke-static {v1}, Ljava/lang/Short;->valueOf(S)Ljava/lang/Short;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/util/HashSet;->add(Ljava/lang/Object;)Z

    sget-object v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->by:Ljava/util/HashSet;

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->f:I

    int-to-short v1, v1

    invoke-static {v1}, Ljava/lang/Short;->valueOf(S)Ljava/lang/Short;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/util/HashSet;->add(Ljava/lang/Object;)Z

    sget-object v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->by:Ljava/util/HashSet;

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->C:I

    int-to-short v1, v1

    invoke-static {v1}, Ljava/lang/Short;->valueOf(S)Ljava/lang/Short;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/util/HashSet;->add(Ljava/lang/Object;)Z

    sget-object v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->by:Ljava/util/HashSet;

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->A:I

    int-to-short v1, v1

    invoke-static {v1}, Ljava/lang/Short;->valueOf(S)Ljava/lang/Short;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/util/HashSet;->add(Ljava/lang/Object;)Z

    sget-object v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->by:Ljava/util/HashSet;

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->M:I

    int-to-short v1, v1

    invoke-static {v1}, Ljava/lang/Short;->valueOf(S)Ljava/lang/Short;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/util/HashSet;->add(Ljava/lang/Object;)Z

    new-instance v0, Ljava/util/HashSet;

    sget-object v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->by:Ljava/util/HashSet;

    invoke-direct {v0, v1}, Ljava/util/HashSet;-><init>(Ljava/util/Collection;)V

    sput-object v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bx:Ljava/util/HashSet;

    const/4 v1, -0x1

    invoke-static {v1}, Ljava/lang/Short;->valueOf(S)Ljava/lang/Short;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/util/HashSet;->add(Ljava/lang/Object;)Z

    sget-object v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bx:Ljava/util/HashSet;

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->D:I

    int-to-short v1, v1

    invoke-static {v1}, Ljava/lang/Short;->valueOf(S)Ljava/lang/Short;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/util/HashSet;->add(Ljava/lang/Object;)Z

    sget-object v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bx:Ljava/util/HashSet;

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->L:I

    int-to-short v1, v1

    invoke-static {v1}, Ljava/lang/Short;->valueOf(S)Ljava/lang/Short;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/util/HashSet;->add(Ljava/lang/Object;)Z

    const-wide/16 v0, 0x64

    invoke-static {v0, v1}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v0

    sput-object v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->a:Ljava/lang/Long;

    sput-object v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->V:Ljava/lang/Long;

    sget-object v0, Ljava/nio/ByteOrder;->BIG_ENDIAN:Ljava/nio/ByteOrder;

    sput-object v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->U:Ljava/nio/ByteOrder;

    return-void
.end method

.method public constructor <init>()V
    .locals 2
    .annotation build Lcom/google/android/apps/common/proguard/UsedByReflection;
    .end annotation

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    const/4 v0, 0x0

    iput v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->Q:I

    new-instance v0, Lkkd;

    sget-object v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->U:Ljava/nio/ByteOrder;

    invoke-direct {v0, v1}, Lkkd;-><init>(Ljava/nio/ByteOrder;)V

    iput-object v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->R:Lkkd;

    new-instance v0, Ljava/text/SimpleDateFormat;

    const-string v1, "yyyy:MM:dd HH:mm:ss"

    invoke-direct {v0, v1}, Ljava/text/SimpleDateFormat;-><init>(Ljava/lang/String;)V

    iput-object v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bw:Ljava/text/DateFormat;

    new-instance v0, Ljava/text/SimpleDateFormat;

    const-string v1, "yyyy:MM:dd"

    invoke-direct {v0, v1}, Ljava/text/SimpleDateFormat;-><init>(Ljava/lang/String;)V

    iput-object v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->S:Ljava/text/DateFormat;

    const-string v0, "UTC"

    invoke-static {v0}, Ljava/util/TimeZone;->getTimeZone(Ljava/lang/String;)Ljava/util/TimeZone;

    move-result-object v0

    invoke-static {v0}, Ljava/util/Calendar;->getInstance(Ljava/util/TimeZone;)Ljava/util/Calendar;

    move-result-object v0

    iput-object v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->T:Ljava/util/Calendar;

    const/4 v0, 0x0

    iput-object v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bz:Landroid/util/SparseIntArray;

    iget-object v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->S:Ljava/text/DateFormat;

    const-string v1, "UTC"

    invoke-static {v1}, Ljava/util/TimeZone;->getTimeZone(Ljava/lang/String;)Ljava/util/TimeZone;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/text/DateFormat;->setTimeZone(Ljava/util/TimeZone;)V

    return-void
.end method

.method private static a([I)I
    .locals 8

    const/4 v1, 0x0

    array-length v0, p0

    if-eqz v0, :cond_2

    sget-object v4, Lkkq;->c:[I

    move v3, v1

    move v0, v1

    :goto_0
    const/4 v2, 0x5

    if-ge v3, v2, :cond_3

    array-length v5, p0

    move v2, v1

    :goto_1
    if-ge v2, v5, :cond_0

    aget v6, p0, v2

    aget v7, v4, v3

    if-ne v7, v6, :cond_1

    const/4 v2, 0x1

    shl-int/2addr v2, v3

    or-int/2addr v0, v2

    :cond_0
    add-int/lit8 v2, v3, 0x1

    move v3, v2

    goto :goto_0

    :cond_1
    add-int/lit8 v2, v2, 0x1

    goto :goto_1

    :cond_2
    move v0, v1

    :cond_3
    return v0
.end method

.method public static a(II)Z
    .locals 6

    const/4 v2, 0x1

    const/4 v1, 0x0

    ushr-int/lit8 v3, p0, 0x18

    sget-object v4, Lkkq;->c:[I

    move v0, v1

    :goto_0
    array-length v5, v4

    if-ge v0, v5, :cond_2

    aget v5, v4, v0

    if-eq p1, v5, :cond_1

    :cond_0
    add-int/lit8 v0, v0, 0x1

    goto :goto_0

    :cond_1
    shr-int v5, v3, v0

    and-int/lit8 v5, v5, 0x1

    if-ne v5, v2, :cond_0

    move v1, v2

    :cond_2
    return v1
.end method

.method public static a(S)Z
    .locals 2

    sget-object v0, Lcom/google/android/libraries/camera/exif/ExifInterface;->by:Ljava/util/HashSet;

    invoke-static {p0}, Ljava/lang/Short;->valueOf(S)Ljava/lang/Short;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/util/HashSet;->contains(Ljava/lang/Object;)Z

    move-result v0

    return v0
.end method

.method public static a(D)[Lkhn;
    .locals 14

    const-wide/16 v12, 0x1

    const-wide/high16 v10, 0x404e000000000000L    # 60.0

    invoke-static {p0, p1}, Ljava/lang/Math;->abs(D)D

    move-result-wide v0

    double-to-int v2, v0

    int-to-double v4, v2

    sub-double/2addr v0, v4

    mul-double/2addr v0, v10

    double-to-int v3, v0

    sget-object v4, Lcom/google/android/libraries/camera/exif/ExifInterface;->V:Ljava/lang/Long;

    invoke-virtual {v4}, Ljava/lang/Long;->longValue()J

    move-result-wide v4

    const/4 v6, 0x3

    new-array v6, v6, [Lkhn;

    new-instance v7, Lkhn;

    int-to-long v8, v2

    invoke-direct {v7, v8, v9, v12, v13}, Lkhn;-><init>(JJ)V

    const/4 v2, 0x0

    aput-object v7, v6, v2

    new-instance v2, Lkhn;

    int-to-long v8, v3

    invoke-direct {v2, v8, v9, v12, v13}, Lkhn;-><init>(JJ)V

    const/4 v7, 0x1

    aput-object v2, v6, v7

    new-instance v2, Lkhn;

    int-to-double v8, v3

    sub-double/2addr v0, v8

    mul-double/2addr v0, v10

    long-to-double v4, v4

    mul-double/2addr v0, v4

    double-to-int v0, v0

    int-to-long v0, v0

    sget-object v3, Lcom/google/android/libraries/camera/exif/ExifInterface;->V:Ljava/lang/Long;

    invoke-virtual {v3}, Ljava/lang/Long;->longValue()J

    move-result-wide v4

    invoke-direct {v2, v0, v1, v4, v5}, Lkhn;-><init>(JJ)V

    const/4 v0, 0x2

    aput-object v2, v6, v0

    return-object v6
.end method

.method private final b(II)Lkkn;
    .locals 3

    const/4 v0, 0x0

    invoke-static {p2}, Lkkn;->a(I)Z

    move-result v1

    if-eqz v1, :cond_0

    int-to-short v1, p1

    iget-object v2, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->R:Lkkd;

    iget-object v2, v2, Lkkd;->b:[Lkkq;

    aget-object v2, v2, p2

    if-eqz v2, :cond_0

    invoke-virtual {v2, v1}, Lkkq;->a(S)Lkkn;

    move-result-object v0

    :cond_0
    return-object v0
.end method

.method public static b(Ljava/io/InputStream;Ljava/io/OutputStream;)V
    .locals 5

    const/16 v4, 0x400

    const/4 v3, 0x0

    new-array v1, v4, [B

    invoke-virtual {p0, v1, v3, v4}, Ljava/io/InputStream;->read([BII)I

    move-result v0

    :goto_0
    const/4 v2, -0x1

    if-eq v0, v2, :cond_0

    invoke-virtual {p1, v1, v3, v0}, Ljava/io/OutputStream;->write([BII)V

    invoke-virtual {p0, v1, v3, v4}, Ljava/io/InputStream;->read([BII)I

    move-result v0

    goto :goto_0

    :cond_0
    return-void
.end method

.method public static c(I)S
    .locals 1

    int-to-short v0, p0

    return v0
.end method

.method private final c(II)[Lkhn;
    .locals 1

    invoke-direct {p0, p1, p2}, Lcom/google/android/libraries/camera/exif/ExifInterface;->b(II)Lkkn;

    move-result-object v0

    if-eqz v0, :cond_0

    invoke-virtual {v0}, Lkkn;->e()[Lkhn;

    move-result-object v0

    :goto_0
    return-object v0

    :cond_0
    const/4 v0, 0x0

    goto :goto_0
.end method

.method private static h(I)S
    .locals 1

    shr-int/lit8 v0, p0, 0x10

    and-int/lit16 v0, v0, 0xff

    int-to-short v0, v0

    return v0
.end method


# virtual methods
.method public final a()Landroid/util/SparseIntArray;
    .locals 14

    const/high16 v9, 0x40000

    const/high16 v13, 0x30000

    const/high16 v12, 0x20000

    const/4 v11, 0x0

    const/4 v10, 0x1

    iget-object v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bz:Landroid/util/SparseIntArray;

    if-nez v0, :cond_0

    new-instance v0, Landroid/util/SparseIntArray;

    invoke-direct {v0}, Landroid/util/SparseIntArray;-><init>()V

    iput-object v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bz:Landroid/util/SparseIntArray;

    const/4 v0, 0x2

    new-array v0, v0, [I

    aput v11, v0, v11

    aput v10, v0, v10

    invoke-static {v0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a([I)I

    move-result v0

    shl-int/lit8 v1, v0, 0x18

    iget-object v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bz:Landroid/util/SparseIntArray;

    invoke-static {v0}, Lmef;->a(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Landroid/util/SparseIntArray;

    or-int v2, v1, v12

    sget v3, Lcom/google/android/libraries/camera/exif/ExifInterface;->E:I

    invoke-virtual {v0, v3, v2}, Landroid/util/SparseIntArray;->put(II)V

    or-int v3, v1, v9

    or-int/lit8 v4, v3, 0x1

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->z:I

    invoke-virtual {v0, v5, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->y:I

    invoke-virtual {v0, v5, v4}, Landroid/util/SparseIntArray;->put(II)V

    or-int v5, v1, v13

    sget v6, Lcom/google/android/libraries/camera/exif/ExifInterface;->X:I

    or-int/lit8 v7, v5, 0x3

    invoke-virtual {v0, v6, v7}, Landroid/util/SparseIntArray;->put(II)V

    or-int/lit8 v6, v5, 0x1

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->ad:I

    invoke-virtual {v0, v7, v6}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->aU:I

    invoke-virtual {v0, v7, v6}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->H:I

    invoke-virtual {v0, v7, v6}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->bb:I

    invoke-virtual {v0, v7, v6}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->aV:I

    invoke-virtual {v0, v7, v6}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->bu:I

    or-int/lit8 v8, v5, 0x2

    invoke-virtual {v0, v7, v8}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->bt:I

    invoke-virtual {v0, v7, v6}, Landroid/util/SparseIntArray;->put(II)V

    const/high16 v7, 0x50000

    or-int/2addr v1, v7

    or-int/lit8 v7, v1, 0x1

    sget v8, Lcom/google/android/libraries/camera/exif/ExifInterface;->br:I

    invoke-virtual {v0, v8, v7}, Landroid/util/SparseIntArray;->put(II)V

    sget v8, Lcom/google/android/libraries/camera/exif/ExifInterface;->bv:I

    invoke-virtual {v0, v8, v7}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->aZ:I

    invoke-virtual {v0, v7, v6}, Landroid/util/SparseIntArray;->put(II)V

    sget v6, Lcom/google/android/libraries/camera/exif/ExifInterface;->M:I

    invoke-virtual {v0, v6, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v6, Lcom/google/android/libraries/camera/exif/ExifInterface;->ba:I

    invoke-virtual {v0, v6, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v6, Lcom/google/android/libraries/camera/exif/ExifInterface;->L:I

    invoke-virtual {v0, v6, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v3, Lcom/google/android/libraries/camera/exif/ExifInterface;->bo:I

    or-int/lit16 v5, v5, 0x300

    invoke-virtual {v0, v3, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v3, Lcom/google/android/libraries/camera/exif/ExifInterface;->bq:I

    or-int/lit8 v5, v1, 0x2

    invoke-virtual {v0, v3, v5}, Landroid/util/SparseIntArray;->put(II)V

    or-int/lit8 v3, v1, 0x6

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->aW:I

    invoke-virtual {v0, v5, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->bs:I

    or-int/lit8 v1, v1, 0x3

    invoke-virtual {v0, v5, v1}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->aX:I

    invoke-virtual {v0, v1, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->c:I

    or-int/lit8 v3, v2, 0x14

    invoke-virtual {v0, v1, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->x:I

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->E:I

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->G:I

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->TAG_SOFTWARE:I

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->W:I

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->af:I

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->f:I

    invoke-virtual {v0, v1, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->p:I

    invoke-virtual {v0, v1, v4}, Landroid/util/SparseIntArray;->put(II)V

    new-array v1, v10, [I

    aput v10, v1, v11

    invoke-static {v1}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a([I)I

    move-result v1

    shl-int/lit8 v1, v1, 0x18

    or-int/2addr v1, v9

    or-int/lit8 v1, v1, 0x1

    sget v2, Lcom/google/android/libraries/camera/exif/ExifInterface;->C:I

    invoke-virtual {v0, v2, v1}, Landroid/util/SparseIntArray;->put(II)V

    sget v2, Lcom/google/android/libraries/camera/exif/ExifInterface;->D:I

    invoke-virtual {v0, v2, v1}, Landroid/util/SparseIntArray;->put(II)V

    new-array v1, v10, [I

    const/4 v2, 0x2

    aput v2, v1, v11

    invoke-static {v1}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a([I)I

    move-result v1

    shl-int/lit8 v1, v1, 0x18

    const/high16 v2, 0x70000

    or-int/2addr v2, v1

    or-int/lit8 v3, v2, 0x4

    sget v4, Lcom/google/android/libraries/camera/exif/ExifInterface;->aj:I

    invoke-virtual {v0, v4, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v4, Lcom/google/android/libraries/camera/exif/ExifInterface;->ao:I

    invoke-virtual {v0, v4, v3}, Landroid/util/SparseIntArray;->put(II)V

    or-int v4, v1, v13

    or-int/lit8 v5, v4, 0x1

    sget v6, Lcom/google/android/libraries/camera/exif/ExifInterface;->aa:I

    invoke-virtual {v0, v6, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v6, Lcom/google/android/libraries/camera/exif/ExifInterface;->ab:I

    invoke-virtual {v0, v6, v3}, Landroid/util/SparseIntArray;->put(II)V

    const/high16 v3, 0x50000

    or-int/2addr v3, v1

    or-int/lit8 v3, v3, 0x1

    sget v6, Lcom/google/android/libraries/camera/exif/ExifInterface;->ac:I

    invoke-virtual {v0, v6, v3}, Landroid/util/SparseIntArray;->put(II)V

    or-int v6, v1, v9

    or-int/lit8 v6, v6, 0x1

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->I:I

    invoke-virtual {v0, v7, v6}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->J:I

    invoke-virtual {v0, v7, v6}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->F:I

    invoke-virtual {v0, v7, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->bp:I

    invoke-virtual {v0, v7, v2}, Landroid/util/SparseIntArray;->put(II)V

    or-int v7, v1, v12

    sget v8, Lcom/google/android/libraries/camera/exif/ExifInterface;->aY:I

    or-int/lit8 v9, v7, 0xd

    invoke-virtual {v0, v8, v9}, Landroid/util/SparseIntArray;->put(II)V

    or-int/lit8 v8, v7, 0x14

    sget v9, Lcom/google/android/libraries/camera/exif/ExifInterface;->e:I

    invoke-virtual {v0, v9, v8}, Landroid/util/SparseIntArray;->put(II)V

    sget v9, Lcom/google/android/libraries/camera/exif/ExifInterface;->d:I

    invoke-virtual {v0, v9, v8}, Landroid/util/SparseIntArray;->put(II)V

    sget v8, Lcom/google/android/libraries/camera/exif/ExifInterface;->bl:I

    invoke-virtual {v0, v8, v7}, Landroid/util/SparseIntArray;->put(II)V

    sget v8, Lcom/google/android/libraries/camera/exif/ExifInterface;->bn:I

    invoke-virtual {v0, v8, v7}, Landroid/util/SparseIntArray;->put(II)V

    sget v8, Lcom/google/android/libraries/camera/exif/ExifInterface;->bm:I

    invoke-virtual {v0, v8, v7}, Landroid/util/SparseIntArray;->put(II)V

    sget v8, Lcom/google/android/libraries/camera/exif/ExifInterface;->aN:I

    or-int/lit8 v9, v7, 0x21

    invoke-virtual {v0, v8, v9}, Landroid/util/SparseIntArray;->put(II)V

    sget v8, Lcom/google/android/libraries/camera/exif/ExifInterface;->h:I

    invoke-virtual {v0, v8, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v8, Lcom/google/android/libraries/camera/exif/ExifInterface;->l:I

    invoke-virtual {v0, v8, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v8, Lcom/google/android/libraries/camera/exif/ExifInterface;->am:I

    invoke-virtual {v0, v8, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v8, Lcom/google/android/libraries/camera/exif/ExifInterface;->bi:I

    invoke-virtual {v0, v8, v7}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->B:I

    invoke-virtual {v0, v7, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->aT:I

    invoke-virtual {v0, v7, v2}, Landroid/util/SparseIntArray;->put(II)V

    const/high16 v7, 0xa0000

    or-int/2addr v1, v7

    or-int/lit8 v1, v1, 0x1

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->K:I

    invoke-virtual {v0, v7, v1}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->b:I

    invoke-virtual {v0, v7, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->Y:I

    invoke-virtual {v0, v7, v1}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->g:I

    invoke-virtual {v0, v7, v1}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->aR:I

    invoke-virtual {v0, v1, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->N:I

    invoke-virtual {v0, v1, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->aS:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->aQ:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->i:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->j:I

    invoke-virtual {v0, v1, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->bj:I

    invoke-virtual {v0, v1, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->ap:I

    invoke-virtual {v0, v1, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->bh:I

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->ar:I

    invoke-virtual {v0, v1, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->as:I

    invoke-virtual {v0, v1, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->aq:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->bk:I

    or-int/lit8 v4, v4, 0x2

    invoke-virtual {v0, v1, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->ak:I

    invoke-virtual {v0, v1, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->bf:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    or-int/lit8 v1, v2, 0x1

    sget v4, Lcom/google/android/libraries/camera/exif/ExifInterface;->an:I

    invoke-virtual {v0, v4, v1}, Landroid/util/SparseIntArray;->put(II)V

    sget v4, Lcom/google/android/libraries/camera/exif/ExifInterface;->be:I

    invoke-virtual {v0, v4, v1}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->Z:I

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->ag:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->al:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->P:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->ai:I

    invoke-virtual {v0, v1, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->k:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->bd:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->at:I

    invoke-virtual {v0, v1, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->ae:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->bc:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->bg:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->ah:I

    invoke-virtual {v0, v1, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->O:I

    invoke-virtual {v0, v1, v5}, Landroid/util/SparseIntArray;->put(II)V

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->A:I

    invoke-virtual {v0, v1, v6}, Landroid/util/SparseIntArray;->put(II)V

    new-array v1, v10, [I

    const/4 v2, 0x4

    aput v2, v1, v11

    invoke-static {v1}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a([I)I

    move-result v1

    shl-int/lit8 v1, v1, 0x18

    const/high16 v2, 0x10000

    or-int/2addr v2, v1

    sget v3, Lcom/google/android/libraries/camera/exif/ExifInterface;->aM:I

    or-int/lit8 v4, v2, 0x4

    invoke-virtual {v0, v3, v4}, Landroid/util/SparseIntArray;->put(II)V

    or-int v3, v1, v12

    or-int/lit8 v4, v3, 0x2

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->t:I

    invoke-virtual {v0, v5, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->v:I

    invoke-virtual {v0, v5, v4}, Landroid/util/SparseIntArray;->put(II)V

    const/high16 v5, 0x50000

    or-int/2addr v5, v1

    or-int/lit8 v6, v5, 0x3

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->s:I

    invoke-virtual {v0, v7, v6}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->u:I

    invoke-virtual {v0, v7, v6}, Landroid/util/SparseIntArray;->put(II)V

    sget v7, Lcom/google/android/libraries/camera/exif/ExifInterface;->n:I

    or-int/lit8 v2, v2, 0x1

    invoke-virtual {v0, v7, v2}, Landroid/util/SparseIntArray;->put(II)V

    or-int/lit8 v2, v5, 0x1

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->m:I

    invoke-virtual {v0, v5, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->w:I

    invoke-virtual {v0, v5, v6}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->aG:I

    invoke-virtual {v0, v5, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->aJ:I

    invoke-virtual {v0, v5, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->aE:I

    invoke-virtual {v0, v5, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->aC:I

    invoke-virtual {v0, v5, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->aI:I

    invoke-virtual {v0, v5, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->aH:I

    invoke-virtual {v0, v5, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->aL:I

    invoke-virtual {v0, v5, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->aK:I

    invoke-virtual {v0, v5, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->r:I

    invoke-virtual {v0, v5, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->q:I

    invoke-virtual {v0, v5, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->aD:I

    invoke-virtual {v0, v5, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->aA:I

    invoke-virtual {v0, v5, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->az:I

    invoke-virtual {v0, v5, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->aw:I

    invoke-virtual {v0, v5, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->av:I

    invoke-virtual {v0, v5, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v5, Lcom/google/android/libraries/camera/exif/ExifInterface;->ay:I

    invoke-virtual {v0, v5, v4}, Landroid/util/SparseIntArray;->put(II)V

    sget v4, Lcom/google/android/libraries/camera/exif/ExifInterface;->ax:I

    invoke-virtual {v0, v4, v2}, Landroid/util/SparseIntArray;->put(II)V

    const/high16 v2, 0x70000

    or-int/2addr v2, v1

    sget v4, Lcom/google/android/libraries/camera/exif/ExifInterface;->aF:I

    invoke-virtual {v0, v4, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v4, Lcom/google/android/libraries/camera/exif/ExifInterface;->au:I

    invoke-virtual {v0, v4, v2}, Landroid/util/SparseIntArray;->put(II)V

    sget v2, Lcom/google/android/libraries/camera/exif/ExifInterface;->o:I

    or-int/lit8 v3, v3, 0xb

    invoke-virtual {v0, v2, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v2, Lcom/google/android/libraries/camera/exif/ExifInterface;->aB:I

    or-int/2addr v1, v13

    or-int/lit8 v1, v1, 0xb

    invoke-virtual {v0, v2, v1}, Landroid/util/SparseIntArray;->put(II)V

    new-array v1, v10, [I

    const/4 v2, 0x3

    aput v2, v1, v11

    invoke-static {v1}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a([I)I

    move-result v1

    shl-int/lit8 v1, v1, 0x18

    sget v2, Lcom/google/android/libraries/camera/exif/ExifInterface;->aO:I

    or-int v3, v1, v12

    invoke-virtual {v0, v2, v3}, Landroid/util/SparseIntArray;->put(II)V

    sget v2, Lcom/google/android/libraries/camera/exif/ExifInterface;->aP:I

    const/high16 v3, 0x70000

    or-int/2addr v1, v3

    invoke-virtual {v0, v2, v1}, Landroid/util/SparseIntArray;->put(II)V

    :cond_0
    iget-object v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bz:Landroid/util/SparseIntArray;

    invoke-static {v0}, Lmef;->a(Ljava/lang/Object;)Ljava/lang/Object;

    iget-object v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bz:Landroid/util/SparseIntArray;

    return-object v0
.end method

.method public final a(Ljava/io/OutputStream;)Ljava/io/OutputStream;
    .locals 4

    if-eqz p1, :cond_0

    new-instance v0, Lkan;

    new-instance v1, Lkko;

    new-instance v2, Ljava/io/BufferedOutputStream;

    const/high16 v3, 0x10000

    invoke-direct {v2, p1, v3}, Ljava/io/BufferedOutputStream;-><init>(Ljava/io/OutputStream;I)V

    iget-object v3, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->R:Lkkd;

    invoke-direct {v1, v2, p0, v3}, Lkko;-><init>(Ljava/io/OutputStream;Lcom/google/android/libraries/camera/exif/ExifInterface;Lkkd;)V

    invoke-direct {v0, v1}, Lkan;-><init>(Lkao;)V

    return-object v0

    :cond_0
    new-instance v0, Ljava/lang/IllegalArgumentException;

    const-string v1, "Argument is null"

    invoke-direct {v0, v1}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw v0
.end method

.method public final a(Ljava/lang/String;)Ljava/io/OutputStream;
    .locals 2

    if-eqz p1, :cond_0

    :try_start_0
    new-instance v0, Ljava/io/FileOutputStream;

    invoke-direct {v0, p1}, Ljava/io/FileOutputStream;-><init>(Ljava/lang/String;)V
    :try_end_0
    .catch Ljava/io/FileNotFoundException; {:try_start_0 .. :try_end_0} :catch_0

    invoke-virtual {p0, v0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a(Ljava/io/OutputStream;)Ljava/io/OutputStream;

    move-result-object v0

    return-object v0

    :cond_0
    new-instance v0, Ljava/lang/IllegalArgumentException;

    const-string v1, "Argument is null"

    invoke-direct {v0, v1}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw v0

    :catch_0
    move-exception v0

    throw v0
.end method

.method public final a(I)Lkhn;
    .locals 3

    invoke-virtual {p0, p1}, Lcom/google/android/libraries/camera/exif/ExifInterface;->f(I)I

    move-result v0

    invoke-direct {p0, p1, v0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->c(II)[Lkhn;

    move-result-object v1

    if-eqz v1, :cond_0

    array-length v0, v1

    if-eqz v0, :cond_0

    new-instance v0, Lkhn;

    const/4 v2, 0x0

    aget-object v1, v1, v2

    invoke-direct {v0, v1}, Lkhn;-><init>(Lkhn;)V

    :goto_0
    return-object v0

    :cond_0
    const/4 v0, 0x0

    goto :goto_0
.end method

.method public final a(ILjava/lang/Object;)Lkkn;
    .locals 9

    const/4 v8, 0x0

    const/4 v6, 0x1

    const/4 v7, 0x0

    ushr-int/lit8 v4, p1, 0x10

    invoke-virtual {p0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a()Landroid/util/SparseIntArray;

    move-result-object v0

    invoke-virtual {v0, p1}, Landroid/util/SparseIntArray;->get(I)I

    move-result v0

    if-eqz v0, :cond_18

    if-eqz p2, :cond_18

    invoke-static {v0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->h(I)S

    move-result v2

    int-to-char v3, v0

    if-eqz v3, :cond_17

    move v5, v6

    :goto_0
    invoke-static {v0, v4}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a(II)Z

    move-result v0

    if-eqz v0, :cond_16

    new-instance v0, Lkkn;

    int-to-short v1, p1

    invoke-direct/range {v0 .. v5}, Lkkn;-><init>(SSIIZ)V

    if-eqz p2, :cond_15

    instance-of v1, p2, Ljava/lang/Short;

    if-eqz v1, :cond_0

    check-cast p2, Ljava/lang/Short;

    invoke-virtual {p2}, Ljava/lang/Short;->shortValue()S

    move-result v1

    int-to-char v1, v1

    invoke-virtual {v0, v1}, Lkkn;->b(I)Z

    move-result v1

    :goto_1
    if-eqz v1, :cond_15

    :goto_2
    return-object v0

    :cond_0
    instance-of v1, p2, Ljava/lang/String;

    if-eqz v1, :cond_1

    check-cast p2, Ljava/lang/String;

    invoke-virtual {v0, p2}, Lkkn;->a(Ljava/lang/String;)Z

    move-result v1

    goto :goto_1

    :cond_1
    instance-of v1, p2, [I

    if-eqz v1, :cond_2

    check-cast p2, [I

    invoke-virtual {v0, p2}, Lkkn;->a([I)Z

    move-result v1

    goto :goto_1

    :cond_2
    instance-of v1, p2, [J

    if-eqz v1, :cond_3

    check-cast p2, [J

    invoke-virtual {v0, p2}, Lkkn;->a([J)Z

    move-result v1

    goto :goto_1

    :cond_3
    instance-of v1, p2, Lkhn;

    if-eqz v1, :cond_4

    check-cast p2, Lkhn;

    new-array v1, v6, [Lkhn;

    aput-object p2, v1, v7

    invoke-virtual {v0, v1}, Lkkn;->a([Lkhn;)Z

    move-result v1

    goto :goto_1

    :cond_4
    instance-of v1, p2, [Lkhn;

    if-eqz v1, :cond_5

    check-cast p2, [Lkhn;

    invoke-virtual {v0, p2}, Lkkn;->a([Lkhn;)Z

    move-result v1

    goto :goto_1

    :cond_5
    instance-of v1, p2, [B

    if-eqz v1, :cond_6

    check-cast p2, [B

    invoke-virtual {v0, p2}, Lkkn;->a([B)Z

    move-result v1

    goto :goto_1

    :cond_6
    instance-of v1, p2, Ljava/lang/Integer;

    if-eqz v1, :cond_7

    check-cast p2, Ljava/lang/Integer;

    invoke-virtual {p2}, Ljava/lang/Integer;->intValue()I

    move-result v1

    invoke-virtual {v0, v1}, Lkkn;->b(I)Z

    move-result v1

    goto :goto_1

    :cond_7
    instance-of v1, p2, Ljava/lang/Long;

    if-eqz v1, :cond_8

    check-cast p2, Ljava/lang/Long;

    invoke-virtual {p2}, Ljava/lang/Long;->longValue()J

    move-result-wide v2

    new-array v1, v6, [J

    aput-wide v2, v1, v7

    invoke-virtual {v0, v1}, Lkkn;->a([J)Z

    move-result v1

    goto :goto_1

    :cond_8
    instance-of v1, p2, Ljava/lang/Byte;

    if-eqz v1, :cond_9

    check-cast p2, Ljava/lang/Byte;

    invoke-virtual {p2}, Ljava/lang/Byte;->byteValue()B

    move-result v1

    new-array v2, v6, [B

    aput-byte v1, v2, v7

    invoke-virtual {v0, v2}, Lkkn;->a([B)Z

    move-result v1

    goto :goto_1

    :cond_9
    instance-of v1, p2, [Ljava/lang/Short;

    if-eqz v1, :cond_c

    check-cast p2, [Ljava/lang/Short;

    array-length v1, p2

    new-array v3, v1, [I

    move v1, v7

    :goto_3
    array-length v2, p2

    if-ge v1, v2, :cond_b

    aget-object v2, p2, v1

    if-eqz v2, :cond_a

    invoke-virtual {v2}, Ljava/lang/Short;->shortValue()S

    move-result v2

    int-to-char v2, v2

    :goto_4
    aput v2, v3, v1

    add-int/lit8 v1, v1, 0x1

    goto :goto_3

    :cond_a
    move v2, v7

    goto :goto_4

    :cond_b
    invoke-virtual {v0, v3}, Lkkn;->a([I)Z

    move-result v1

    goto/16 :goto_1

    :cond_c
    instance-of v1, p2, [Ljava/lang/Integer;

    if-eqz v1, :cond_f

    check-cast p2, [Ljava/lang/Integer;

    array-length v1, p2

    new-array v3, v1, [I

    move v1, v7

    :goto_5
    array-length v2, p2

    if-ge v1, v2, :cond_e

    aget-object v2, p2, v1

    if-eqz v2, :cond_d

    invoke-virtual {v2}, Ljava/lang/Integer;->intValue()I

    move-result v2

    :goto_6
    aput v2, v3, v1

    add-int/lit8 v1, v1, 0x1

    goto :goto_5

    :cond_d
    move v2, v7

    goto :goto_6

    :cond_e
    invoke-virtual {v0, v3}, Lkkn;->a([I)Z

    move-result v1

    goto/16 :goto_1

    :cond_f
    instance-of v1, p2, [Ljava/lang/Long;

    if-eqz v1, :cond_12

    check-cast p2, [Ljava/lang/Long;

    array-length v1, p2

    new-array v1, v1, [J

    :goto_7
    array-length v2, p2

    if-ge v7, v2, :cond_11

    aget-object v2, p2, v7

    if-eqz v2, :cond_10

    invoke-virtual {v2}, Ljava/lang/Long;->longValue()J

    move-result-wide v2

    :goto_8
    aput-wide v2, v1, v7

    add-int/lit8 v7, v7, 0x1

    goto :goto_7

    :cond_10
    const-wide/16 v2, 0x0

    goto :goto_8

    :cond_11
    invoke-virtual {v0, v1}, Lkkn;->a([J)Z

    move-result v1

    goto/16 :goto_1

    :cond_12
    instance-of v1, p2, [Ljava/lang/Byte;

    if-eqz v1, :cond_15

    check-cast p2, [Ljava/lang/Byte;

    array-length v1, p2

    new-array v3, v1, [B

    move v1, v7

    :goto_9
    array-length v2, p2

    if-ge v1, v2, :cond_14

    aget-object v2, p2, v1

    if-eqz v2, :cond_13

    invoke-virtual {v2}, Ljava/lang/Byte;->byteValue()B

    move-result v2

    :goto_a
    aput-byte v2, v3, v1

    add-int/lit8 v1, v1, 0x1

    goto :goto_9

    :cond_13
    move v2, v7

    goto :goto_a

    :cond_14
    invoke-virtual {v0, v3}, Lkkn;->a([B)Z

    move-result v1

    goto/16 :goto_1

    :cond_15
    move-object v0, v8

    goto/16 :goto_2

    :cond_16
    move-object v0, v8

    goto/16 :goto_2

    :cond_17
    move v5, v7

    goto/16 :goto_0

    :cond_18
    move-object v0, v8

    goto/16 :goto_2
.end method

.method public final a(Lkkn;)Lkkn;
    .locals 1

    iget-object v0, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->R:Lkkd;

    invoke-virtual {v0, p1}, Lkkd;->a(Lkkn;)Lkkn;

    move-result-object v0

    return-object v0
.end method

.method public final a(Ljava/io/InputStream;)V
    .locals 8

    const/4 v1, 0x0

    if-eqz p1, :cond_a

    :try_start_0
    new-instance v0, Lkkm;

    invoke-direct {v0, p0}, Lkkm;-><init>(Lcom/google/android/libraries/camera/exif/ExifInterface;)V

    iget-object v0, v0, Lkkm;->a:Lcom/google/android/libraries/camera/exif/ExifInterface;

    new-instance v2, Lkki;

    invoke-direct {v2, p1, v0}, Lkki;-><init>(Ljava/io/InputStream;Lcom/google/android/libraries/camera/exif/ExifInterface;)V

    new-instance v3, Lkkd;

    iget-object v0, v2, Lkki;->g:Lkkc;

    iget-object v0, v0, Lkkc;->a:Ljava/nio/ByteBuffer;

    invoke-virtual {v0}, Ljava/nio/ByteBuffer;->order()Ljava/nio/ByteOrder;

    move-result-object v0

    invoke-direct {v3, v0}, Lkkd;-><init>(Ljava/nio/ByteOrder;)V

    invoke-virtual {v2}, Lkki;->a()I

    move-result v0

    :goto_0
    const/4 v4, 0x5

    if-eq v0, v4, :cond_9

    packed-switch v0, :pswitch_data_0

    :cond_0
    :goto_1
    invoke-virtual {v2}, Lkki;->a()I

    move-result v0

    goto :goto_0

    :pswitch_0
    iget-object v0, v2, Lkki;->e:Lkkn;

    if-eqz v0, :cond_4

    const/4 v4, 0x0

    invoke-virtual {v0, v4}, Lkkn;->c(I)J

    move-result-wide v4

    long-to-int v0, v4

    :goto_2
    new-array v4, v0, [B

    array-length v0, v4

    invoke-virtual {v2, v4}, Lkki;->a([B)I

    move-result v5

    if-ne v0, v5, :cond_3

    iget-object v0, v2, Lkki;->b:Lkkl;

    iget v5, v0, Lkkl;->a:I

    iget-object v0, v3, Lkkd;->c:Ljava/util/ArrayList;

    invoke-virtual {v0}, Ljava/util/ArrayList;->size()I

    move-result v0

    if-ge v5, v0, :cond_1

    iget-object v0, v3, Lkkd;->c:Ljava/util/ArrayList;

    invoke-virtual {v0, v5, v4}, Ljava/util/ArrayList;->set(ILjava/lang/Object;)Ljava/lang/Object;
    :try_end_0
    .catch Lkkf; {:try_start_0 .. :try_end_0} :catch_0

    goto :goto_1

    :catch_0
    move-exception v0

    new-instance v1, Ljava/io/IOException;

    const-string v2, "Invalid exif format : "

    invoke-direct {v1, v2, v0}, Ljava/io/IOException;-><init>(Ljava/lang/String;Ljava/lang/Throwable;)V

    throw v1

    :cond_1
    :try_start_1
    iget-object v0, v3, Lkkd;->c:Ljava/util/ArrayList;

    invoke-virtual {v0}, Ljava/util/ArrayList;->size()I

    move-result v0

    :goto_3
    if-ge v0, v5, :cond_2

    iget-object v6, v3, Lkkd;->c:Ljava/util/ArrayList;

    const/4 v7, 0x0

    invoke-virtual {v6, v7}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    add-int/lit8 v0, v0, 0x1

    goto :goto_3

    :cond_2
    iget-object v0, v3, Lkkd;->c:Ljava/util/ArrayList;

    invoke-virtual {v0, v4}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    goto :goto_1

    :cond_3
    const-string v0, "CAM_ExifReader"

    const-string v4, "Failed to read the strip bytes"

    invoke-static {v0, v4}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    goto :goto_1

    :cond_4
    move v0, v1

    goto :goto_2

    :pswitch_1
    iget-object v0, v2, Lkki;->c:Lkkn;

    if-eqz v0, :cond_6

    const/4 v4, 0x0

    invoke-virtual {v0, v4}, Lkkn;->c(I)J

    move-result-wide v4

    long-to-int v0, v4

    :goto_4
    new-array v0, v0, [B

    array-length v4, v0

    invoke-virtual {v2, v0}, Lkki;->a([B)I

    move-result v5

    if-eq v4, v5, :cond_5

    const-string v0, "CAM_ExifReader"

    const-string v4, "Failed to read the compressed thumbnail"

    invoke-static {v0, v4}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    goto :goto_1

    :cond_5
    iput-object v0, v3, Lkkd;->d:[B

    goto :goto_1

    :cond_6
    move v0, v1

    goto :goto_4

    :pswitch_2
    iget-object v0, v2, Lkki;->f:Lkkn;

    if-eqz v0, :cond_0

    iget-short v4, v0, Lkkn;->b:S

    const/4 v5, 0x7

    if-eq v4, v5, :cond_7

    :goto_5
    iget v4, v0, Lkkn;->d:I

    invoke-virtual {v3, v4}, Lkkd;->b(I)Lkkq;

    move-result-object v4

    if-eqz v4, :cond_0

    invoke-virtual {v4, v0}, Lkkq;->a(Lkkn;)Lkkn;

    goto/16 :goto_1

    :cond_7
    invoke-virtual {v2, v0}, Lkki;->a(Lkkn;)V

    goto :goto_5

    :pswitch_3
    iget-object v0, v2, Lkki;->f:Lkkn;

    if-eqz v0, :cond_0

    invoke-virtual {v0}, Lkkn;->b()Z

    move-result v4

    if-nez v4, :cond_8

    iget v4, v0, Lkkn;->e:I

    iget-object v5, v2, Lkki;->g:Lkkc;

    iget v5, v5, Lkkc;->b:I

    if-lt v4, v5, :cond_0

    iget-object v5, v2, Lkki;->d:Ljava/util/TreeMap;

    invoke-static {v4}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v4

    new-instance v6, Lkkj;

    const/4 v7, 0x1

    invoke-direct {v6, v0, v7}, Lkkj;-><init>(Lkkn;Z)V

    invoke-virtual {v5, v4, v6}, Ljava/util/TreeMap;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    goto/16 :goto_1

    :cond_8
    iget v4, v0, Lkkn;->d:I

    invoke-virtual {v3, v4}, Lkkd;->b(I)Lkkq;

    move-result-object v4

    if-eqz v4, :cond_0

    invoke-virtual {v4, v0}, Lkkq;->a(Lkkn;)Lkkn;

    goto/16 :goto_1

    :pswitch_4
    new-instance v0, Lkkq;

    iget v4, v2, Lkki;->a:I

    invoke-direct {v0, v4}, Lkkq;-><init>(I)V

    invoke-virtual {v3, v0}, Lkkd;->a(Lkkq;)V
    :try_end_1
    .catch Lkkf; {:try_start_1 .. :try_end_1} :catch_0

    goto/16 :goto_1

    :cond_9
    iput-object v3, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->R:Lkkd;

    return-void

    :cond_a
    new-instance v0, Ljava/lang/IllegalArgumentException;

    const-string v1, "Argument is null"

    invoke-direct {v0, v1}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw v0

    nop

    :pswitch_data_0
    .packed-switch 0x0
        :pswitch_4
        :pswitch_3
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch
.end method

.method public final a(Ljava/io/InputStream;Ljava/io/OutputStream;)V
    .locals 2

    if-eqz p1, :cond_0

    invoke-virtual {p0, p2}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a(Ljava/io/OutputStream;)Ljava/io/OutputStream;

    move-result-object v0

    invoke-static {p1, v0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->b(Ljava/io/InputStream;Ljava/io/OutputStream;)V

    invoke-virtual {v0}, Ljava/io/OutputStream;->flush()V

    invoke-virtual {v0}, Ljava/io/OutputStream;->close()V

    return-void

    :cond_0
    new-instance v0, Ljava/lang/IllegalArgumentException;

    const-string v1, "Argument is null"

    invoke-direct {v0, v1}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw v0
.end method

.method public final a([B)V
    .locals 1

    new-instance v0, Ljava/io/ByteArrayInputStream;

    invoke-direct {v0, p1}, Ljava/io/ByteArrayInputStream;-><init>([B)V

    invoke-virtual {p0, v0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a(Ljava/io/InputStream;)V

    return-void
.end method

.method public final a(IJLjava/util/TimeZone;)Z
    .locals 4

    const/4 v0, 0x0

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->c:I

    if-eq p1, v1, :cond_0

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->d:I

    if-eq p1, v1, :cond_0

    sget v1, Lcom/google/android/libraries/camera/exif/ExifInterface;->e:I

    if-ne p1, v1, :cond_1

    :cond_0
    iget-object v1, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bw:Ljava/text/DateFormat;

    invoke-virtual {v1, p4}, Ljava/text/DateFormat;->setTimeZone(Ljava/util/TimeZone;)V

    iget-object v1, p0, Lcom/google/android/libraries/camera/exif/ExifInterface;->bw:Ljava/text/DateFormat;

    invoke-static {p2, p3}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v2

    invoke-virtual {v1, v2}, Ljava/text/DateFormat;->format(Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v1

    invoke-virtual {p0, p1, v1}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a(ILjava/lang/Object;)Lkkn;

    move-result-object v1

    if-eqz v1, :cond_1

    invoke-virtual {p0, v1}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a(Lkkn;)Lkkn;

    const/4 v0, 0x1

    :cond_1
    return v0
.end method

.method public final b(I)Ljava/lang/Integer;
    .locals 3

    const/4 v0, 0x0

    invoke-virtual {p0, p1}, Lcom/google/android/libraries/camera/exif/ExifInterface;->f(I)I

    move-result v1

    invoke-direct {p0, p1, v1}, Lcom/google/android/libraries/camera/exif/ExifInterface;->b(II)Lkkn;

    move-result-object v1

    if-eqz v1, :cond_1

    invoke-virtual {v1}, Lkkn;->g()[I

    move-result-object v1

    :goto_0
    if-eqz v1, :cond_0

    array-length v2, v1

    if-lez v2, :cond_0

    const/4 v0, 0x0

    aget v0, v1, v0

    invoke-static {v0}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v0

    :cond_0
    return-object v0

    :cond_1
    move-object v1, v0

    goto :goto_0
.end method

.method public final d(I)Lkkn;
    .locals 1

    invoke-virtual {p0, p1}, Lcom/google/android/libraries/camera/exif/ExifInterface;->f(I)I

    move-result v0

    invoke-direct {p0, p1, v0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->b(II)Lkkn;

    move-result-object v0

    return-object v0
.end method

.method public final e(I)[Lkhn;
    .locals 1

    invoke-virtual {p0, p1}, Lcom/google/android/libraries/camera/exif/ExifInterface;->f(I)I

    move-result v0

    invoke-direct {p0, p1, v0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->c(II)[Lkhn;

    move-result-object v0

    return-object v0
.end method

.method public final f(I)I
    .locals 1

    invoke-virtual {p0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a()Landroid/util/SparseIntArray;

    move-result-object v0

    invoke-virtual {v0, p1}, Landroid/util/SparseIntArray;->get(I)I

    move-result v0

    if-eqz v0, :cond_0

    ushr-int/lit8 v0, p1, 0x10

    :goto_0
    return v0

    :cond_0
    const/4 v0, -0x1

    goto :goto_0
.end method

.method public final g(I)Lkkn;
    .locals 6

    invoke-virtual {p0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a()Landroid/util/SparseIntArray;

    move-result-object v0

    invoke-virtual {v0, p1}, Landroid/util/SparseIntArray;->get(I)I

    move-result v0

    if-eqz v0, :cond_1

    invoke-static {v0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->h(I)S

    move-result v2

    int-to-char v3, v0

    if-eqz v3, :cond_0

    const/4 v5, 0x1

    :goto_0
    new-instance v0, Lkkn;

    int-to-short v1, p1

    ushr-int/lit8 v4, p1, 0x10

    invoke-direct/range {v0 .. v5}, Lkkn;-><init>(SSIIZ)V

    :goto_1
    return-object v0

    :cond_0
    const/4 v5, 0x0

    goto :goto_0

    :cond_1
    const/4 v0, 0x0

    goto :goto_1
.end method

.method public getTagStringValue(I)Ljava/lang/String;
    .locals 1
    .annotation build Lcom/google/android/apps/common/proguard/UsedByReflection;
    .end annotation

    invoke-virtual {p0, p1}, Lcom/google/android/libraries/camera/exif/ExifInterface;->f(I)I

    move-result v0

    invoke-direct {p0, p1, v0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->b(II)Lkkn;

    move-result-object v0

    if-eqz v0, :cond_0

    invoke-virtual {v0}, Lkkn;->c()Ljava/lang/String;

    move-result-object v0

    :goto_0
    return-object v0

    :cond_0
    const/4 v0, 0x0

    goto :goto_0
.end method

.method public readExif(Ljava/lang/String;)V
    .locals 2
    .annotation build Lcom/google/android/apps/common/proguard/UsedByReflection;
    .end annotation

    if-eqz p1, :cond_0

    new-instance v0, Ljava/io/BufferedInputStream;

    new-instance v1, Ljava/io/FileInputStream;

    invoke-direct {v1, p1}, Ljava/io/FileInputStream;-><init>(Ljava/lang/String;)V

    invoke-direct {v0, v1}, Ljava/io/BufferedInputStream;-><init>(Ljava/io/InputStream;)V

    invoke-virtual {p0, v0}, Lcom/google/android/libraries/camera/exif/ExifInterface;->a(Ljava/io/InputStream;)V

    invoke-virtual {v0}, Ljava/io/InputStream;->close()V

    return-void

    :cond_0
    new-instance v0, Ljava/lang/IllegalArgumentException;

    const-string v1, "Argument is null"

    invoke-direct {v0, v1}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw v0
.end method
